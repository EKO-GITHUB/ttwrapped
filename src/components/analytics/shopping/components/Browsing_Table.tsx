...
import { browsing_history_columns } from "./browsing-columns";
import { Product_Browsing_History_Item } from "@/types/TikTok_Data_Schema";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";

export default function Browsing_Table({ browsing_history }: { browsing_history: Product_Browsing_History_Item[] }) {
  const [selected_shop, set_selected_shop] = useState<string>("all");

  const available_shops = useMemo(() => calculate_available_shops(browsing_history), [browsing_history]);

  const filtered_items = useMemo(
    () => apply_shop_filter(browsing_history, selected_shop),
    [browsing_history, selected_shop],
  );

  const columns = useMemo(() => browsing_history_columns, []);
      ...
      <div className="flex items-center justify-between">
        <h4 className="flex items-center gap-2 font-semibold text-gray-900 dark:text-gray-100">
          ...
          <span className="text-sm font-normal text-gray-500 dark:text-gray-400">
            ({filtered_items.length} of {browsing_history.length})
          </span>
        </h4>
      </div>
...
function calculate_available_shops(browsing_history: Product_Browsing_History_Item[]) {
  const shops = new Set(browsing_history.map((item) => item.shop_name).filter((shop) => shop && shop.trim() !== ""));
  return Array.from(shops).sort();
}

function apply_shop_filter(browsing_history: Product_Browsing_History_Item[], selected_shop: string) {
  if (selected_shop === "all") {
    return browsing_history;
  }
  return browsing_history.filter((item) => item.shop_name === selected_shop);
}
