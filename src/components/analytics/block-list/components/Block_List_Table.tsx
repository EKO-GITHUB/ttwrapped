...
import { ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, UserX } from "lucide-react";
import { block_list_table_columns } from "./Block_List_Table_Columns";
import { Block_List_Item } from "@/types/TikTok_Data_Schema";

export default function Block_List_Table({ blocked_users }: { blocked_users: Block_List_Item[] }) {
  const columns = useMemo(() => block_list_table_columns, []);
    ...
    <div className="grid grid-cols-1 gap-4">
      <div className="flex items-center justify-between">
        <h4 className="flex items-center gap-2 font-semibold text-gray-900 dark:text-gray-100">
          ...
          Blocked Users
          <span className="text-sm font-normal text-gray-500 dark:text-gray-400">({blocked_users.length})</span>
        </h4>
      </div>
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center justify-between border-t border-gray-200 bg-gray-50 px-4 py-3 dark:border-gray-700 dark:bg-gray-800/50">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-500 dark:text-gray-400">({blocked_users.length} total)</span>
          </div>
