...
export function Background_Direct_Messages() {
  const message_paths_ref = useRef<Message_Path[]>([]);
  const message_dots_ref = useRef<Message_Dot[]>([]);
  const frame_count_ref = useRef(0);

  const colors = ["#FF6B9D", "#4ECDC4", "#45B7D1", "#A29BFE", "#74B9FF", "#55E6C1", "#FD79A8", "#F8B500"];

  const initialize_dots = (canvas: HTMLCanvasElement) => {
    const dots: Message_Dot[] = [];
    const padding = 80;
    const dot_count = 6;
    ...
    message_dots_ref.current = dots;
  };

  const create_message_path = (canvas: HTMLCanvasElement): Message_Path => {
    const dots = message_dots_ref.current;
    const start_side = Math.random() > 0.5 ? "left" : "right";
    const left_dots = dots.filter((d) => d.side === "left");
    const right_dots = dots.filter((d) => d.side === "right");
  ...
  const get_point_on_curve = (points: { x: number; y: number }[], t: number) => {
    if (points.length === 2) {
  ...
  const draw_message_dot = (ctx: CanvasRenderingContext2D, dot: Message_Dot, frame_count: number) => {
  ...
  const draw_message_path = (ctx: CanvasRenderingContext2D, path: Message_Path) => {
  ...
  const update_message_path = (path: Message_Path) => {
    ...
    path.lifetime += 1;
    return path.progress <= 1.3 && path.lifetime <= path.max_lifetime;
  };

  const draw = (ctx: CanvasRenderingContext2D) => {
    const canvas = ctx.canvas;
    if (message_dots_ref.current.length === 0) initialize_dots(canvas);

    frame_count_ref.current++;
    const frame_count = frame_count_ref.current;

    ctx.clearRect(0, 0, canvas.width, canvas.height);

    if (frame_count % 40 === 0 && message_paths_ref.current.length < 8) {
      message_paths_ref.current.push(create_message_path(canvas));
    }

    message_dots_ref.current.forEach((dot) => draw_message_dot(ctx, dot, frame_count));

    for (let i = message_paths_ref.current.length - 1; i >= 0; i--) {
      const path = message_paths_ref.current[i];
      if (!update_message_path(path)) {
        message_paths_ref.current.splice(i, 1);
      } else {
        draw_message_path(ctx, path);
      }
    }
  };
